...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql+asyncpg://postgres:postgres@postgres:5432/infra_audit_db"
)
...
async def cache_job_status(job_id: str, status: str):
    try:
        ...
        pass  # never break DB operations if Redis is down
...
# -------------------------------------------------------------------
# PostgreSQL: Insert or Update Request
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# PostgreSQL Logs Resources (EC2, VPC, etc)
# -------------------------------------------------------------------
    ...
    async with async_session() as session:
        async with session.begin():
            ...
            if not exists.scalar():
                raise ValueError(f"log_id {log_id} does not exist")
